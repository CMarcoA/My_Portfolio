...
export default function MainPage({ shouldFadeIn = false }) {
  ...
  useEffect(() => {
    ...
    // Skip fade if returning from a detail page or if shouldFadeIn is handling it
    if (!shouldFadeIn) {
      if (!hasShownMainPage) {
  ...
  return (
    <div className={`mp-root-wrapper ${isVisible ? 'fade-in' : ''}`}>
